...
const file = fs.readFileSync("../test/savegames/vortex_lothern_turn_0_v1_5_1.save");
...
const result = regionsArray.map(entry => {
  ...
  const settlementX = settlement_expansion_manager[4] * (3376 / 725);  // x position of settlement at index 4 in SETTLEMENT_EXPANSION_MANAGER node
  const settlementY = 3868 - (settlement_expansion_manager[5] * (3868 / 720));  // y position of settlement at index 5 in SETTLEMENT_EXPANSION_MANAGER node
...
fsExtra.outputJSONSync("vortex_settlements_positions.json", result);
